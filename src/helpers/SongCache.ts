...
export default class SongCache {
    ...
    public constructor(options: CacheOptions) {
        ...
        .then(status => {
            if (!status && this.web) window.addEventListener('online', () => this.checkForNew())
            this.is_ready = true
    ...
    public async checkForNew() {
        ...
        while (true) {
            ...
            const newSongs = songs.filter(song => !this.songs.find(s => s.songUrl == song.songUrl))
            if (this.detailed_log) console.log(`added ${newSongs.length} new songs of page ${cp} | song names: [${newSongs.map(s => s.name).join(', ')}]`)

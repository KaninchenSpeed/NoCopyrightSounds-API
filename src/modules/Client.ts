...
export default class NCSClient {
    ...
    public async getSongs(page = 1) {
        if (this.cache) {
            ...
            return this.cache.songs.slice(start, Math.min(start + 20, this.cache.songs.length))
        } else {
            if (this.web) {
    ...
    public async search(filter: Filter, page = 1) {
        if (this.cache && this.detailed_log) {
            console.warn('This method is not cached! Use an alterate search engine insted like https://www.npmjs.com/package/flexsearch')
        }
        if (this.web) {
